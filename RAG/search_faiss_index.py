...
class LawFAISSSearcher:
    ...
    def _load_all(self):
        ...
        if os.path.exists(texts_path):
            ...
            print(f"文本数据加载完成，共 {len(self.texts)} 条")
        else:
        ...
        if os.path.exists(metadata_path):
            ...
            print(f"元数据加载完成，共 {len(self.metadata)} 条")
        else:
    ...
    def pretty_print_results(self, results: List[Dict[str, Any]]):
        ...
        for result in results:
            print(f"\n【排名 {result['rank']}】 相似度: {result['score']:.4f}")
            print(f"编章: {result['part_title']} - {result['chapter_title']}")
            print(f"条文: {result['article_no']}")
            print(f"内容: {result['article_content']}")
            print("-" * 80)
