    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                # 显示结果
                print(f"\n📊 检索结果 (共找到 {response['retrieved_count']} 条相关法条):")
                print("=" * 60)
...
def batch_test():
    ...
    try:
        ...
        for i, query in enumerate(test_queries, 1):
            ...
            print(f"问题: {query}")
            print(f"找到相关法条: {response['retrieved_count']} 条")
            
            if response['results']:
                best_result = response['results'][0]
                print(f"最相关法条: {best_result['article_no']}")
                print(f"相关度: {best_result['score']:.3f}")
                print(f"内容摘要: {best_result['article_content'][:50]}...")
