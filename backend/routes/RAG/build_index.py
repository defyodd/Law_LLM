...
class LawFAISSIndexBuilder:
    def __init__(self, model_name: str = 'paraphrase-multilingual-MiniLM-L12-v2'):
        print(f"正在加载模型: {model_name}", flush=True)
    ...
    def load_json_data(self, json_file_path: str) -> List[Dict[str, Any]]:
        ...
        并包含文件级别 title
        """
        print(f"正在加载JSON文件: {json_file_path}", flush=True)
        ...
        print(f"文件 {os.path.basename(json_file_path)} 中处理了 {len(processed_data)} 条文", flush=True)
        return processed_data

    def build_embeddings(self, texts: List[str]) -> np.ndarray:
        print("正在构建文本嵌入向量...", flush=True)
        embeddings = self.model.encode(texts, show_progress_bar=True)
        return embeddings

    def build_faiss_index(self, embeddings: np.ndarray) -> faiss.IndexFlatIP:
        print("正在构建FAISS索引...", flush=True)
        faiss.normalize_L2(embeddings)
        dimension = embeddings.shape[1]
        index = faiss.IndexFlatIP(dimension)
        index.add(embeddings)
        print(f"FAISS索引构建完成，包含 {index.ntotal} 个向量", flush=True)
        return index
    ...
    def save_index(self, index: faiss.IndexFlatIP, texts: List[str], metadata: List[Dict], save_dir: str):
        ...
        print(f"FAISS索引及配套文件已保存到 {save_dir}", flush=True)
    ...
    def build_index_from_json_dir(self, json_dir_path: str, save_dir: str):
        ...
        print("FAISS 索引构建完成！", flush=True)


def main():
    json_dir = "Law_LLM/crawled data/cleaned_data"
    save_dir = "Law_LLM/RAG/indexes"
